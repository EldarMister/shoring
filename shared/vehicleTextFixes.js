...
const TITLE_REPLACEMENTS = [
  ...
  // Korean romanizations previously caught only by scripts/fix-car-fields.js.
  // Promoting them to the live normalizer so fresh scrapes auto-clean.
  [/\bteuraebeoseu\b/gi, 'Traverse'],
  [/\bhaikeonteuri\b/gi, 'High Country'],
  [/\bmaenyupaekcheo\b/gi, 'MANUFAKTUR'],
  [/\bipeopomeonseu\b/gi, 'iPerformance'],
  [/\b4mosyeon\b/gi, '4Motion'],
  [/\bdilraiteu\b/gi, 'Delight'],
  ...
  [/\beorinibohocha\b/gi, 'School Bus'],
  [/\bpeulreokseu\b/gi, 'Flux'],
  [/\bikseuteurim\b/gi, 'Extreme'],
  [/\bredeupit\b/gi, 'Red Fit'],
  [/\bpeibeodeu\b/gi, 'Paved'],
  // "Redeo" = 레더 (Leather) for Chevrolet Traverse LT Leather Premium. Guarded
  // to trim context to avoid clashing with unrelated tokens.
  [/\bredeo\s+premium\b/gi, 'Leather Premium'],
  [/\bredeo\b(?=\s)/gi, 'Leather'],
]
...
const TRIM_REPLACEMENTS = [
  ...
  // Mirror TITLE-level Korean romanizations into TRIM_REPLACEMENTS so trims
  // pulled from the Encar grade text also normalize.
  [/\bteuraebeoseu\b/gi, 'Traverse'],
  [/\bhaikeonteuri\b/gi, 'High Country'],
  [/\bmaenyupaekcheo\b/gi, 'MANUFAKTUR'],
  [/\bipeopomeonseu\b/gi, 'iPerformance'],
